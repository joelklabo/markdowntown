...
import { CITY_WORDMARK_EVENT, dispatchCityWordmarkEvent, listenCityWordmarkEvents } from "@/components/wordmark/sim/events";
import { useCityWordmarkSim } from "@/components/wordmark/sim/useCityWordmarkSim";
import type { CityWordmarkDensity } from "@/components/wordmark/sim/types";
      ...
      <div className="space-y-mdt-4">
        ...
        <Card className="p-mdt-4 space-y-mdt-3">
          ...
          <div className="grid grid-cols-2 gap-mdt-2">
            <Button
              variant="secondary"
              onClick={() => dispatchCityWordmarkEvent({ type: "search", query: "skyline", ts: Date.now() })}
            >
              Search
            </Button>
            <Button
              variant="secondary"
              onClick={() => dispatchCityWordmarkEvent({ type: "command_palette_open", origin: "storybook", ts: Date.now() })}
            >
              Command
            </Button>
            <Button
              variant="secondary"
              onClick={() => dispatchCityWordmarkEvent({ type: "publish", kind: "artifact", ts: Date.now() })}
            >
              Publish
            </Button>
            <Button
              variant="secondary"
              onClick={() => dispatchCityWordmarkEvent({ type: "alert", kind: "ambulance", ts: Date.now() })}
            >
              Ambulance
            </Button>
          </div>
